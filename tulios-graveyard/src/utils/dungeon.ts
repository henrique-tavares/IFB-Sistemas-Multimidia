...
import { CustomBoundConfig, CustomBounds, NextRoom, NextRoomArrowPosition, NextRoomData, NextRoomDest, NextRoomPlayerCoordinate, Orientation, PlayerCoordinate } from '../types';
import NextRoomArrow from "./nextRoomArrow";
import Screen from './screen';

export function handleNextRoomArrows(key: string, scene: Scene, screen: Screen, nextRoom: NextRoom, position: NextRoomArrowPosition){
  console.log(key);
  console.log(Object.entries(nextRoom));
  
  const nextRoomArrows = Object.entries(nextRoom).map(
    ([key, value]) =>{
      return new NextRoomArrow(scene, screen, key as Orientation, Array.isArray(value) ? value.length : 1, position[key] ?? 0)
  });
...
export function addCustomBounds(player: GameObjects.GameObject, scene: Scene, screen: Screen,  customBound: CustomBounds) {
...
export function generateCustomBounds(screen: Screen, paddings: {top: number, bottom: number, horizontal: number}, config: { [Key in Orientation]?: number }): CustomBounds{
  const bound = {
  ...
  return {
    up: config?.up != undefined ? bound.up(config.up) : undefined,
    right: config?.right != undefined ? bound.right(config.right) : undefined,
    down: config?.down != undefined ? bound.down(config.down) : undefined,
    left: config?.left != undefined ? bound.left(config.left) : undefined,
  }
}